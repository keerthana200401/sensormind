...
from backend.schemas import SensorInput, PredictionResponse
...
def predict_engine(payload: SensorInput, db: Session = Depends(get_db)):
    # Load real sample if no sensor data provided
    df = pd.read_parquet(
    ...
    # Save to DB
    saved = crud.save_prediction(db, prediction_data)
